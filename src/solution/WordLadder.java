...
public class WordLadder {
    ...
    class Path {
        String w;
        Path last;
        Path(String w, Path last, int changeIndex){
            this.w = w;
            this.last = last;
        }
    }
    ...
    public int ladderLength(String start, String end, Set<String> dict) {
        ...
        while (!queue.isEmpty()) {
            ...
            //if (almostSame(cur, end)) // next line is faster
            ...
            for (int i = 0; i < cur.length(); i++) {
                ...
                for (char c = 'a'; c <= 'z'; c++) {
                    if (c != ch) {
                        ...
                        if (dict.contains(changed) && !map.containsKey(changed)) {
                            queue.add(changed);
                            map.put(changed, map.get(cur)+1);
                        }
                    }
                }
                sb.setCharAt(i, ch);
